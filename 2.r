# 2
# Турчак Карина создайте модель множественной линейной регрессии потоков 
# потоков паров воды за осенний период 2013 года по данным измерений методом турбулентной пульсации
...
mod2 = lm ( h2o_flux~ DOY + file_records + Tau+qc_Tau + rand_err_Tau + H +qc_H 
            ...
            + rand_err_h2o_flux + H_strg + co2_v_minus_adv + h2o_v_minus_adv
            ...
            + Tau_scf + un_H + H_scf + un_LE + LE_scf + un_h2o_flux + w_spikes + co2_spikes 
            + h2o_spikes + u_var + v_var + co2_var + h2o_var + w_div_h2o_cov + co2 + h2o 
            + h2o_signal_strength_7200 + flowrate, data = teaching_tbl)
...
mod3 = lm ( h2o_flux~ DOY + file_records + Tau + qc_Tau + rand_err_Tau + H
            ...
            + h2o_mole_fraction + h2o_mixing_ratio + h2o_molar_density, data = teaching_tbl)
...
cor_teaching_tbl = select(teaching_tbl, DOY, file_records, Tau, qc_Tau, rand_err_Tau, H,
                          ...
                          rand_err_co2_flux, h2o_flux, rand_err_h2o_flux, H_strg,
                          co2_v_minus_adv, h2o_v_minus_adv, co2_molar_density,
                          h2o_mole_fraction, h2o_mixing_ratio, h2o_molar_density)
